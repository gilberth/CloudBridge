...
1. `pnpm run typecheck` — tsc --noEmit en todos los workspaces (api, web, shared). **Ya hay ~80 errores
...
2. `pnpm run lint` — eslint sobre todo el repo. Nuevos errores (no warnings preexistentes) bloquean.
3. `pnpm run build:shared` — necesario antes de `test` si no corriste `dev`/`build` en esta sesión:
   `apps/api` resuelve `@cloudbridge/shared` en runtime de los tests, no solo en tipos.
4. `pnpm run test` — vitest run en `apps/api` (cliente rclone, saneador de rutas, mapeo de opciones de
   transferencia).
5. `pnpm run build` — build completo (shared → api → web) para atrapar errores que typecheck aislado no ve
...
No corras `pnpm run test:e2e` como parte de esta verificación estándar — requiere el stack de Docker
...
diferencias de estilo) — no corras `pnpm run format` sobre todo el repo como parte de esta verificación,
