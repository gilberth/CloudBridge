...
describe("RcloneClient", () => {
  ...
  it("sends HTTP Basic auth built from user and password", async () => {
    ...
    expect(headers.authorization).toBe(
      `Basic ${Buffer.from("cloudbridge:secret").toString("base64")}`,
    );
  });

  it("posts to <url>/<endpoint> with a JSON body", async () => {
    await client(fetchMock as unknown as typeof fetch).call("operations/list", {
      fs: "disco:",
      remote: "",
    });
  ...
  it("callAsync returns the jobid from an _async response", async () => {
    ...
    const jobId = await client(fetchMock as unknown as typeof fetch).callAsync(
      "sync/copy",
      {
        srcFs: "a:",
        dstFs: "b:",
  ...
  it("maps a 401/403 to a credentials-rejected message, hiding rclone internals", async () => {
    ...
    await expect(
      ...
      message: "Credenciales de la RC API rechazadas",
    });
  });
  ...
  it("obscures plain secrets when creating a remote from the GUI", async () => {
    await client(fetchMock as unknown as typeof fetch).configCreate(
      "icloud",
      "iclouddrive",
      {
  ...
  it("check() runs asynchronously and obtains its result from job/status", async () => {
    fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ jobid: 42 }), {
          headers: { "content-type": "application/json" },
        }),
      )
      .mockResolvedValueOnce(
    ...
    const result = await client(fetchMock as unknown as typeof fetch).check(
      "a:",
      "b:",
      {},
      { group: "run:compare-1" },
