...
export async function settingsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post(
    ...
    async (request) => {
      const input = notificationChannelInputSchema.parse(
        (request.body as { channel?: unknown })?.channel,
      );
      const channel = app.settings.previewNotificationChannel(input);
      await app.notifications.test(channel);
      app.logs.write("info", "notification", `Prueba enviada a ${channel.name}`, {
