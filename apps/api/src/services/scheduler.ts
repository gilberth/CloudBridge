...
export class Scheduler {
  ...
  register(job: Job): void {
    ...
    if (!cron.validate(job.cron)) {
      this.app.logs.write(
        "error",
        "scheduler",
        `Cron inválido en "${job.name}": ${job.cron}`,
        undefined,
        {
          jobId: job.id,
        },
  ...
  private async fire(jobId: string): Promise<void> {
    ...
    const active = this.app.runs
      .active()
      .find((run) => run.jobId === jobId && run.status === "running");
    if (active) {
      this.app.logs.write(
        "warn",
        "scheduler",
    ...
    try {
      ...
      this.app.logs.write(
        "info",
        "scheduler",
        `"${job.name}" lanzado por programación`,
        undefined,
        {
          jobId,
          runId: run.id,
      ...
      this.app.logs.write(
        "error",
        "scheduler",
        `"${job.name}" no pudo lanzarse: ${message}`,
        undefined,
        {
          jobId,
        },
      );
      void this.app.notifications.send(null, "error", message, [], job);
    }
  }
}
