...
export const api = {
  ...
  remotes: {
    ...
    get: (name: string) =>
      request<RemoteDetail>(`/api/remotes/${encodeURIComponent(name)}`),
    create: (input: {
      name: string;
      type: string;
    ...
    update: (
      name: string,
      input: { parameters: Record<string, string>; token?: string },
    ...
    importConfig: (config: string) =>
      post<{ imported: number }>("/api/remotes/config/import", { config }),
  },
  ...
  fs: {
    ...
    mkdir: (remote: string, path: string) =>
      post<void>("/api/fs/mkdir", { remote, path }),
  ...
  transfers: {
    ...
    retryFailures: (id: string, failureIds?: string[]) =>
      post<Run>(`/api/transfers/${encodeURIComponent(id)}/retry-failures`, {
        failureIds,
      }),
  },
  ...
  jobs: {
    ...
    update: (id: string, input: unknown) =>
      put<Job>(`/api/jobs/${encodeURIComponent(id)}`, input),
    ...
    history: (id: string) =>
      request<Run[]>(`/api/jobs/${encodeURIComponent(id)}/history`),
  ...
  settings: {
    ...
    testNotification: (channel: NotificationChannelInput) =>
      post<{ delivered: boolean }>("/api/settings/notifications/test", { channel }),
  },
};
